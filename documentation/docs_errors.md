...
Tools are available in common GIS packages to help repair geometries so that they can be imported into Marxan Web. For ArcGIS, the geoprocessing tool called 'Repair Geometry' can be used. See [here](https://pro.arcgis.com/en/pro-app/tool-reference/data-management/repair-geometry.htm) for more information. The OGC validation method is likely to be more successful in repairing the geometry so that it can be imported into Marxan Web. For QGIS, you can use the 'Fix geometries' tool in the Vector geometry toolbox. See [here](https://docs.qgis.org/testing/en/docs/user_manual/processing_algs/qgis/vectorgeometry.html#qgisfixgeometries) for more information. Finally, if you are using PostGIS, you can fix topological errors using the ST_MakeValid function. See [here](https://postgis.net/docs/ST_MakeValid.html) for more information.
